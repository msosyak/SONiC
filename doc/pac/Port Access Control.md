...
2. In a multi-step process, hostapd runs the 802.1x state machine to Authenticate the client via RADIUS. According to 3.1.1.5 When it receives a PDU, it consults pacd about new client AAA in progress and proceeds. pacd should insert FDB entry according to 2.2.4. (FDB should be added to the APPL db then OA will do the work)
...
4. pacd proceeds to authorize the client. Authenticated clients along with RADIUS authorization parameters like client VLAN membership, are updated in PAC_AUTHENTICATED_CLIENT_OPER table in STATE_DB.
5. VLAN manager listed to updated in PAC_AUTHENTICATED_CLIENT_OPER table in STATE_DB and update VLAN MEMBER TABEL in APPL DB.
TODO FDB<!-- FDB orch further process these APPL DB updates from PAC and write into their STATE_DB and HW. -->
6. Orchagent (VLAN(Port), FDB orch) in SWSS docker gets notified about changes in APPL_DB and responds by translating the APPL_DB changes to respective sairedis calls.
...
9. EAP Success message (EAPoL PDU) is sent to the client. ?????? is it sent by hostapd? how hostabd will now that configuration applied to the hardwer?
...
1. Unknown source MAC packets are received by hardware on a front panel interface and trapped to CPU. The packets gets delivered to a pacd socket. How exactly???? we need to trace mac and port
...
"PAC_PORT_CONFIG": {
 ...
 This ether a typo and it should be "MAB_PORT_CONFIG" or why we need separate table for this? why do not add this fields in "PAC_PORT_CONFIG"
...
dynamic                   =     "yes"/"no"                           ;"yes" = assigned by RADIUS, "no" = configured
tagging_mode              =     "untagged"/"tagged"                  ;Vlan tagging mode

```
!!! Do not understand the need of this "dynamic" field.
```
"PORT_TABLE: {
  "Ethernet1": {
      "learn_mode": "drop",
...
```
!!! ^ do not see tha need in this table
...
Extend Port Manager to set default port learning mode based on 801.x feature enabled or not and according to 2.2.4. I believe this should be done in port manager no avoid unauthorized port access between OA bring up and PACd bring up.

### 3.3.1 Vlan Manager

VLAN Manager processes updates from "pacd" through STATE DB updates and propagates VLAN member updates to APP DB for further processing by OA.

### 3.3.2 Orchestration Agent

OA processes updates from APP DB for setting the Port learning mode, FDB entiries, VLAN membership and PVID and passes down the same to SAI Redis library for updating the ASIC DB.
